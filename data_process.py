...
times_valid = [
    ('2024-03-28 17:54:00', '2024-03-28 20:00:00')
]#

times_test = [
    ('2024-03-28 18:54:00', '2024-03-28 20:10:00')
]#

# save_all_fetch_data(times_train, 1, root_dir='./data/boutique/test/', interval=30, services=services,metrics=metrics)#interval 间隔
# save_all_fetch_data(times_valid, 1, root_dir='./data/boutique/test/', interval=30, services=services,metrics=metrics)#interval 间隔
save_all_fetch_data(times_test, 1, root_dir='./data/boutique/test/', interval=30, services=services,metrics=metrics)#interval 间隔

### pod
# "adservice"
file = './data/boutique/test/1_{}_{}.log'.format(services[0],metrics[0])
adservice_pod=np.genfromtxt(file, dtype=np.double)
adservice_pod = adservice_pod[:,np.newaxis]
# "cartservice"
file = './data/boutique/test/1_{}_{}.log'.format(services[1],metrics[0])
cartservice_pod=np.genfromtxt(file, dtype=np.double)
cartservice_pod = cartservice_pod[:,np.newaxis]
# "checkoutservice"
file = './data/boutique/test/1_{}_{}.log'.format(services[2],metrics[0])
checkoutservice_pod=np.genfromtxt(file, dtype=np.double)
checkoutservice_pod = checkoutservice_pod[:,np.newaxis]
# "currencyservice"
file = './data/boutique/test/1_{}_{}.log'.format(services[3],metrics[0])
currencyservice_pod=np.genfromtxt(file, dtype=np.double)
currencyservice_pod = currencyservice_pod[:,np.newaxis]
# "emailservice"
file = './data/boutique/test/1_{}_{}.log'.format(services[4],metrics[0])
emailservice_pod=np.genfromtxt(file, dtype=np.double)
emailservice_pod = emailservice_pod[:,np.newaxis]
# "frontend"
file = './data/boutique/test/1_{}_{}.log'.format(services[5],metrics[0])
frontend_pod=np.genfromtxt(file, dtype=np.double)
frontend_pod = frontend_pod[:,np.newaxis]
# "paymentservice"
file = './data/boutique/test/1_{}_{}.log'.format(services[6],metrics[0])
paymentservice_pod=np.genfromtxt(file, dtype=np.double)
paymentservice_pod = paymentservice_pod[:,np.newaxis]
# "productcatalogservice"
file = './data/boutique/test/1_{}_{}.log'.format(services[7],metrics[0])
...
file = './data/boutique/test/1_{}_{}.log'.format(services[8],metrics[0])
...
# "recommendationservice"
file = './data/boutique/test/1_{}_{}.log'.format(services[8],metrics[1])
...
# "shippingservice"
file = './data/boutique/test/1_{}_{}.log'.format(services[9],metrics[1])
...
# "recommendationservice"
file = './data/boutique/test/1_{}_{}.log'.format(services[8],metrics[2])
...
# "shippingservice"
file = './data/boutique/test/1_{}_{}.log'.format(services[9],metrics[2])
...
# "recommendationservice"
file = './data/boutique/test/1_{}_{}.log'.format(services[8],metrics[3])
...
# "shippingservice"
file = './data/boutique/test/1_{}_{}.log'.format(services[9],metrics[3])
...
# "recommendationservice"
file = './data/boutique/test/1_{}_{}.log'.format(services[8],metrics[4])
...
# "shippingservice"
file = './data/boutique/test/1_{}_{}.log'.format(services[9],metrics[4])
...
xx = xx.numpy()
print(xx.shape)
np.savez("./data/boutique/test", xx)  # 生成原始的序列数据

## for train 
# all_data = read_and_generate_dataset(graph_signal_matrix_filename='./data/boutique/test.npz', 
#                                         num_of_hours=1, 
#                                         num_for_predict=1, 
#                                         points_per_hour=120, 
#                                         save=True)
...
# 生成验证集数据
# valid_data = read_and_generate_dataset(graph_signal_matrix_filename='./data/boutique/test.npz',
#                                         num_of_hours=1,  # 生成一个小时的数据作为验证集
#                                         num_for_predict=1,  # 预测未来 120 个数据点
#                                         points_per_hour=120,  # 每小时 120 个数据点
#                                         save=True)

# 生成测试集数据
test_data = read_and_generate_dataset(graph_signal_matrix_filename='./data/boutique/test.npz',
                                       num_of_hours=1,
                                       num_for_predict=1,  # 预测未来 1 个数据点
                                       points_per_hour=120,  # 每小时 120 个数据点
                                       save=True)
