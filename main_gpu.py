...
def main(args):
    ...
    data_names = ('train_r24ssj.npz', 'valid_r24ssj.npz', 'test_r24ssj.npz')
    data_loaders = []
    for data_name in data_names:
        dataset = TPDataset(os.path.join(data_config['train_data_dir'], data_name))
        if data_name == 'train_r24ssj.npz':
...
def plot_results(y_true, y_pred, node_index=5, dpi=300, save_path='compare_frontend.png'):
    y_true = y_true[:, :, node_index].flatten() 
    y_pred = y_pred[:, :, node_index].flatten()  
    plt.figure(figsize=(20, 10))  
    plt.plot(y_true, label='True', linestyle='-', color='b')  
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_config_path', type=str, default='config/train_dataset_speed_gpu.yaml',
