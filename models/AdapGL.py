...
class GraphLearn(torch.nn.Module):
    ...
    def __init__(self, num_nodes, init_feature_num):
        ...
        torch.nn.init.kaiming_uniform_(self.w1, a=math.sqrt(5))#卷积初始化
        torch.nn.init.kaiming_uniform_(self.w2, a=math.sqrt(5))

    def forward(self, adj_mx):#graph learn的钱箱传播
...
class AdapGLBlockA(torch.nn.Module):###用到这里了
    ...
    def forward(self, x, adj_mx):
        ...
        x_tat = (torch.matmul(temporal_att, x.reshape(b, c, -1)).reshape(b, c, n_d, f))#空间层面的卷积
        hp = self.graph_conv_p(x_tat.reshape(-1, n_d, f), adj_mx)#用到切比雪夫  为了空间关系，应该是时间关系吧
        hn = self.graph_conv_n(x_tat.reshape(-1, n_d, f), adj_mx.T)# Prediction network module
        h = torch.relu(torch.cat((hp, hn), dim=-1).reshape(b, c, n_d, -1))#hidden  将输入特征映射到一个非线性特征空间,从而可以实现更复杂的表达

        h = self.time_conv(h.transpose(1, 3)).transpose(1, 3)#时间层面的卷积
        h_res = self.residual_conv(x.transpose(1, 3)).transpose(1, 3)#残差卷积

        h = torch.relu(h + h_res)#这里是concat部分
        return self.ln(h)
...
class AdapGLA(torch.nn.Module):
    ...
    def forward(self, x, adj_mx):
        ...
        h = self.final_conv(h).squeeze(dim=-1)#求最后的军机操作对的到生成值
        return h
