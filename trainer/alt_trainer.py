...
class AdapGLTrainer(Trainer):
    ...
    def update_adj_mx_list(self, data_loader, new_adj_mx):#更新了adj
    ...
    def train(self, train_data_loader, eval_data_loader, metrics=('mae', 'rmse', 'mape')):
        ...
        for i in range(self.num_iter):
            ...
            print('Evaluation results of current graph:')#子图设置为3
        ...
        def train_one_epoch(self, data_loader):
            ...
            for _ in range(self.num_iter):
                for x, y in data_loader:
                    x = x.type(torch.float32).to(self.device)
                    y = y.type(torch.float32).to(self.device)
                    adj_mx = self.model(best_adj_mx)#生成了个新的adj
                    pred = self.outer_obj.model_pred(x, adj_mx)#算算新的adj是啥个反映，实际上进去的也是pred的预测板块
                    loss = self.model_loss_func(pred, y)#图学习参数
                    self.optimizer.zero_grad()
                    loss.backward()
                    self.optimizer.step()
